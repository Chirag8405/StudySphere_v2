...
export async function createLecture(
  ...
  try {
    ...
    const { name, subject, schedule_days, schedule_time } = req.body;

    // Validate input
    if (!name || !subject || !schedule_days || !schedule_time) {
      return res.status(400).json({
        error: "Name, subject, schedule days, and schedule time are required",
      });
    }
